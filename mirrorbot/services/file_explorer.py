...
class FileExplorer:
    ...
    async def _action(self, request: web.Request) -> web.Response:
        ...
        if action == "mkdir":
            ...
            await self.upload_callback(session.chat_id, paths)
        else:
...
<style>body{font:14px system-ui;margin:0;background:#f4f5f7;color:#18212f}header{padding:12px 16px;background:#fff;border-bottom:1px solid #d5dae1;display:flex;gap:8px;flex-wrap:wrap;align-items:center}button{padding:7px 10px;border:1px solid #b9c1cc;background:#fff;cursor:pointer}button.primary{background:#1769e0;color:#fff;border-color:#1769e0}main{max-width:1100px;margin:20px auto;padding:0 14px}#crumbs{margin-bottom:12px}table{width:100%;border-collapse:collapse;background:#fff;border:1px solid #d5dae1}th,td{padding:9px;border-bottom:1px solid #e5e8ec;text-align:left}.name{cursor:pointer;color:#1769e0}.muted{color:#667085}dialog{border:1px solid #aaa;padding:20px}#toast{position:fixed;right:16px;bottom:16px;background:#18212f;color:#fff;padding:10px;display:none}</style></head><body>
<header><button onclick="up()">Up</button><button onclick="mkdir()">New folder</button><button onclick="renameOne()">Rename</button><button onclick="copyMove('copy')">Copy</button><button onclick="copyMove('move')">Move</button><button onclick="removeItems()">Delete</button><button onclick="upload()">Upload to Telegram</button><button class="primary" onclick="scan()">Scan Jellyfin</button><span id="timer" class="muted"></span></header>
<main><div id="crumbs"></div><table><thead><tr><th><input type="checkbox" id="all"></th><th>Name</th><th>Type</th><th>Size</th><th></th></tr></thead><tbody id="rows"></tbody></table></main><div id="toast"></div>
<dialog id="extend"><p>This session expires in under one minute.</p><button class="primary" onclick="extendSession()">Extend 5 minutes</button><button onclick="this.closest('dialog').close()">Not now</button></dialog>
<script>const token=location.pathname.split('/').pop();let path='',expiresAt=0,asked=false;const api=(action,data={})=>fetch(`/local/${token}/api/${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(async r=>{if(!r.ok)throw Error(await r.text());return r.json()});const selected=()=>[...document.querySelectorAll('.pick:checked')].map(x=>x.value);function toast(x){let t=document.querySelector('#toast');t.textContent=x;t.style.display='block';setTimeout(()=>t.style.display='none',2500)}async function load(p=path){let r=await fetch(`/local/${token}/api/list?path=${encodeURIComponent(p)}`);if(!r.ok){document.body.innerHTML='<main><h2>Session expired</h2></main>';return}let d=await r.json();path=d.path;expiresAt=d.expiresAt;asked=false;document.querySelector('#crumbs').textContent='/downloads/'+path;document.querySelector('#rows').innerHTML=d.items.map(i=>`<tr><td><input class="pick" type="checkbox" value="${esc(i.path)}"></td><td class="name" onclick="openItem(decodeURIComponent('${enc(i.path)}'),'${i.type}')">${esc(i.name)}</td><td>${i.type}</td><td>${i.size}</td><td>${i.type==='file'?`<a href="/local/${token}/download?path=${encodeURIComponent(i.path)}">Download</a>`:''}</td></tr>`).join('')}function esc(s){return s.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function enc(s){return encodeURIComponent(s).replace(/'/g,'%27')}function openItem(p,t){if(t==='folder')load(p)}function up(){load(path.split('/').slice(0,-1).join('/'))}async function mkdir(){let name=prompt('Folder name');if(name)await act('mkdir',{path,name})}async function renameOne(){let s=selected();if(s.length!==1)return toast('Select one item');let name=prompt('New name',s[0].split('/').pop());if(name)await act('rename',{source:s[0],name})}async function copyMove(a){let s=selected();if(!s.length)return toast('Select items');let destination=prompt('Destination path under downloads',path);if(destination!==null)await act(a,{sources:s,destination})}async function removeItems(){let s=selected();if(!s.length)return toast('Select items');if(confirm('Permanently delete?\n'+s.join('\n')))await act('delete',{sources:s})}async function upload(){let s=selected();if(!s.length)return toast('Select items');await act('upload',{sources:s});toast('Telegram upload tasks started')}async function scan(){await act('scan');toast('Jellyfin scan requested')}async function act(a,d={}){try{await api(a,d);await load()}catch(e){toast(e.message)}}async function extendSession(){let d=await api('extend');expiresAt=d.expiresAt;asked=false;document.querySelector('#extend').close();toast('Session extended')}document.querySelector('#all').onchange=e=>document.querySelectorAll('.pick').forEach(x=>x.checked=e.target.checked);setInterval(()=>{let n=Math.max(0,Math.ceil(expiresAt-Date.now()/1000));document.querySelector('#timer').textContent=`Expires in ${Math.floor(n/60)}:${String(n%60).padStart(2,'0')}`;if(n<=60&&!asked){asked=true;document.querySelector('#extend').showModal()}if(!n){window.close();document.body.innerHTML='<main><h2>Session expired</h2></main>'}},1000);load();</script></body></html>'''
