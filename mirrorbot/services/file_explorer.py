...
.selectionbar{display:none;position:sticky;top:69px;z-index:7;padding:10px;align-items:center;justify-content:space-between;gap:10px;border-color:color-mix(in srgb,var(--primary) 42%,var(--line));background:color-mix(in srgb,var(--primary-soft) 45%,var(--surface))}.selectionbar.active{display:flex}.selection-head{display:flex;align-items:center;gap:8px}.selection-title{font-weight:850;white-space:nowrap}.clear-selection{display:none}.selection-actions,.file-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}.upload-wrap{display:flex;gap:6px;align-items:center}select{min-height:38px;border:1px solid var(--line-strong);border-radius:7px;background:var(--surface);color:var(--text);padding:8px 10px;font:inherit;font-weight:760}
...
@media(max-width:760px){.appbar-inner{display:grid;padding:12px}.brand h1{font-size:20px}.header-actions{justify-content:flex-start}.header-actions button{flex:1 1 auto}.explorer{padding:12px 10px calc(var(--selectionbar-height,0px) + 22px)}.toolbar,.selectionbar{align-items:stretch}.toolbar{display:grid}.toolbar-left,.toolbar-right,.selection-actions{width:100%}.toolbar-left button,.toolbar-right button{flex:1 1 auto}.selectionbar{display:none}.selectionbar.active{display:grid;grid-template-columns:minmax(0,1fr);justify-content:stretch;position:fixed;top:auto;left:10px;right:10px;bottom:max(10px,env(safe-area-inset-bottom));z-index:12;gap:8px;padding:8px 10px;max-height:48vh;overflow:auto}.selection-head{justify-content:space-between;min-height:34px}.selection-title{font-size:14px}.clear-selection{display:inline-flex;min-height:32px;width:auto;padding:5px 10px;font-size:12px}.selection-actions{display:grid;grid-template-columns:minmax(0,1fr);gap:8px}.file-actions{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;width:100%}.file-actions button{width:100%;min-height:36px;padding:6px 4px;font-size:12px}.upload-wrap{width:100%;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:6px}.upload-wrap select{min-width:0;min-height:36px;padding-block:6px}.upload-wrap button{min-height:36px;padding-block:6px}.desktop-clear{display:none}.files-card{background:transparent;border:0;box-shadow:none}.table-wrap{overflow:visible}table{min-width:0;border:0;background:transparent}thead{display:none}tbody{display:grid;gap:8px}tr.row{display:grid;grid-template-columns:40px minmax(0,1fr);gap:0;border:1px solid var(--line);border-radius:8px;background:var(--surface);overflow:hidden}tr.row.selected{border-color:color-mix(in srgb,var(--primary) 42%,var(--line))}.cell{display:block;border:0;padding:10px}.check{width:auto;grid-column:1;grid-row:1 / span 2;display:flex;align-items:flex-start;justify-content:center;padding-top:16px}.type,.size{display:none}.actions{width:auto;text-align:left;grid-column:2;padding-top:0}.actions a{width:100%}.file-main{align-items:flex-start}.file-sub{overflow-wrap:anywhere}.parent-row .actions{display:none}.empty-state{display:block;border:1px solid var(--line);border-radius:8px;background:var(--surface)}}@media(max-width:760px){.header-actions{display:flex;gap:8px;align-items:center}.header-actions .timer{flex:0 0 auto}.header-actions button{flex:0 0 auto;width:auto}.toolbar{padding:10px}.toolbar-left{display:flex;gap:8px}.toolbar-left button{flex:0 0 auto;width:auto;min-height:36px;padding:7px 10px}.toolbar-right{width:auto}.toolbar .hint{display:inline-flex;min-height:30px;align-items:center}.explorer{gap:10px}tr.row{grid-template-columns:36px minmax(0,1fr)}.check{padding-top:14px}.file-icon{width:30px;height:30px}.cell{padding:9px}.file-main{gap:10px}}@media(max-width:430px){.appbar-inner{gap:10px}.header-actions{display:flex;flex-wrap:wrap}.header-actions button{width:auto}.toolbar{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap}.toolbar-left{width:auto;display:flex;flex-wrap:wrap}.toolbar-left button{width:auto}.toolbar-right{width:auto;margin-left:auto}.selection-actions{width:100%}.file-actions{grid-template-columns:repeat(2,minmax(0,1fr))}.upload-wrap{grid-template-columns:minmax(0,1fr) auto}.crumbs{font-size:13px}}
</style>
</head>
<body>
<header class="appbar">
...
<main class="explorer">
  ...
  <section id="selectionBar" class="selectionbar" aria-live="polite">
    <div class="selection-head">
      ...
      <button class="secondary clear-selection" type="button" onclick="clearSelection()">Clear</button>
    </div>
    <div class="selection-actions">
      ...
      <button class="secondary desktop-clear" type="button" onclick="clearSelection()">Clear</button>
    </div>
  </section>
...
function syncSelectionSpace(){const bar=document.querySelector('#selectionBar');const height=bar.classList.contains('active')?Math.ceil(bar.getBoundingClientRect().height):0;document.documentElement.style.setProperty('--selectionbar-height',`${height}px`)}
function updateSelection(origin=null){const count=selected().length;document.querySelector('#selectedCount').textContent=`${count} selected`;document.querySelector('#selectionBar').classList.toggle('active',count>0);document.querySelector('#defaultTools').classList.toggle('hidden',count>0);document.querySelector('#renameBtn').disabled=count!==1;const picks=[...document.querySelectorAll('.pick')];document.querySelector('#all').checked=picks.length>0&&picks.every(x=>x.checked);document.querySelectorAll('tr.row').forEach(row=>{const pick=row.querySelector('.pick');row.classList.toggle('selected',!!pick&&pick.checked)});requestAnimationFrame(()=>{syncSelectionSpace();if(origin&&matchMedia('(max-width:760px)').matches){const row=origin.closest('tr.row');const bar=document.querySelector('#selectionBar');const overlap=row.getBoundingClientRect().bottom-bar.getBoundingClientRect().top+12;if(overlap>0)window.scrollBy({top:overlap,behavior:'smooth'})}})}
async function load(p=path){let r=await fetch(`/local/${token}/api/list?path=${encodeURIComponent(p)}&_=${Date.now()}`,{cache:'no-store'});if(!r.ok){document.body.innerHTML='<main><h2>Session expired</h2></main>';return}let d=await r.json();path=d.path;expiresAt=d.expiresAt;asked=false;document.querySelector('#count').textContent=`${d.items.length} item${d.items.length===1?'':'s'}`;renderBreadcrumbs();const parent=path?`<tr class="row parent-row"><td class="cell check"></td><td class="cell"><div class="file-main"><span class="file-icon folder"></span><div class="file-name"><button type="button" onclick="up()">Parent folder</button><div class="file-sub">Go up one level</div></div></div></td><td class="cell type">Folder</td><td class="cell size">-</td><td class="cell actions"></td></tr>`:'';const items=d.items.map(i=>`<tr class="row"><td class="cell check"><input class="pick" type="checkbox" value="${esc(i.path)}" onchange="updateSelection(this)"></td><td class="cell"><div class="file-main"><span class="file-icon ${i.type==='folder'?'folder':'file'}"></span><div class="file-name"><button type="button" onclick="openItem(decodeURIComponent('${enc(i.path)}'),'${i.type}')">${esc(i.name)}</button><div class="file-sub">${esc(i.path)}</div></div></div></td><td class="cell type">${i.type}</td><td class="cell size">${i.size}</td><td class="cell actions">${i.type==='file'?`<a class="download" href="/local/${token}/download?path=${encodeURIComponent(i.path)}">Download</a>`:''}</td></tr>`).join('');document.querySelector('#rows').innerHTML=parent+(items||`<tr><td class="empty-state" colspan="5"><strong>This folder is empty</strong>Create a folder or move files here.</td></tr>`);document.querySelector('#all').checked=false;updateSelection()}
...
new ResizeObserver(syncSelectionSpace).observe(document.querySelector('#selectionBar'));window.addEventListener('resize',syncSelectionSpace);
