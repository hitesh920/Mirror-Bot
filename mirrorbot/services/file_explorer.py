...
<div class="bar"><div class="actions"><button onclick="mkdir()">New folder</button><button onclick="renameOne()">Rename</button><button onclick="copyMove('copy')">Copy</button><button onclick="copyMove('move')">Move</button><button class="danger" onclick="removeItems()">Delete</button><button onclick="upload()">Upload to Telegram</button><button class="primary" onclick="scan()">Scan Jellyfin</button></div></div>
...
async function upload(){let s=selected();if(!s.length)return toast('Select items');await act('upload',{sources:s});toast('Telegram upload tasks started')}
