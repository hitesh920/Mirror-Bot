...
@media(max-width:700px){{.appbar-inner{{display:grid;padding:12px}}.brand h1{{font-size:20px}}.meta-pills{{justify-content:flex-start}}.shell{{padding:12px 10px 22px}}.toolbar{{display:grid}}.toolbar input{{min-width:0}}.toolbar button{{width:100%}}table{{min-width:0}}thead{{display:none}}tbody{{display:grid;gap:8px;padding:8px}}tr.row{{display:grid;grid-template-columns:1fr;gap:8px;border:1px solid var(--line);border-radius:8px;background:var(--surface-soft);padding:10px}}th,td,.cell{{display:block;border:0;padding:0}}.number{{display:none}}.action{{width:auto;text-align:left}}.primary-link{{width:100%}}}}
</style></head><body>
