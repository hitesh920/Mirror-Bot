...
def test_config_defaults_for_new_tunables(_env):
    ...
    assert config.r2_stall_timeout_seconds == 1800
...
def test_config_reads_tunables_from_env(_env, monkeypatch):
    ...
    monkeypatch.setenv("R2_STALL_TIMEOUT_SECONDS", "900")
    ...
    assert config.r2_stall_timeout_seconds == 900
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("R2_STALL_TIMEOUT_SECONDS", "5", "at least 30"),
...
def test_configure_applies_config_to_engine_modules(monkeypatch):
    ...
    monkeypatch.setattr(transfer_guard, "R2_STALL_TIMEOUT", 0)
    ...
    assert transfer_guard.R2_STALL_TIMEOUT == 84
